...
forfiles /p E:\数据库备份\159数据库备份 /m *.zip /d -30 /c "cmd /c IF @isdir==TRUE RD /S /Q @path" >> %LOG_FILE_159%
echo ===使用forfiles删除228 30天之前的备份压缩文件,/D参数是天数，/m是文件类型=== >> %LOG_FILE_228%
forfiles /p E:\数据库备份\228数据库备份 /m *.zip /d -30 /c "cmd /c IF @isdir==TRUE RD /S /Q @path" >> %LOG_FILE_228%
